...
# Load data
imdb_movies <- read.csv("/Users/alexmccreight/Portfolio/alex-hugosite/content/project/2021-12-17-Bayes/imdb_movies_new.csv")
imdb_movies <- 
  imdb_movies %>% 
  select(-X) %>%  
...
```{r data-viz-1, echo=FALSE, warning = FALSE, message = FALSE}
...
```{r data-viz-2, echo=FALSE, warning = FALSE, message = FALSE}
logbudgetgraph <- imdb_movies %>% 
  ggplot(aes(y = log(domestic), x = log(budget))) +
  geom_point(alpha = 0.3, color = "forestgreen") + 
  geom_smooth() +
  labs(title = " ",x = "Logged Budget", y = "Logged Domestic Box Office Revenue") +
  theme_classic()

averageratinggraph <- imdb_movies %>% 
  ggplot(aes(y = log(domestic), x = averageRating)) +
  geom_point(alpha = 0.3, color = "darkslateblue") + 
  geom_smooth() +
  labs(title = " ",x = "Average Rating", y = "")+
  theme_classic()

numVotesgraph <- imdb_movies %>% 
  ggplot(aes(y = log(domestic), x = log(numVotes))) +
  geom_point(alpha = 0.3, color = "firebrick4") + 
  geom_smooth() + 
  labs(title = " ",x = "Logged Number of Votes", y = "")+
  theme_classic()
...
```{r data-viz-3, echo=FALSE, warning = FALSE, message = FALSE}
directorgraph <- imdb_movies %>% 
  ggplot(aes(x = director_power, y = log(domestic))) +
  geom_boxplot() +
  labs(y="Log Box Office Revenue", x="Top 100 Grossing Director")+ theme_classic()

producergraph <- imdb_movies %>% 
  ggplot(aes(x = producer_power, y = log(domestic))) +
  geom_boxplot() +
  labs(y="", x="Top 100 Grossing Producer")+ theme_classic()

distributorgraph <- imdb_movies %>% 
  ggplot(aes(x = distributor_power, y = log(domestic))) +
  geom_boxplot() +
  labs(y="Log Box Office Revenue", x="Top 10 Grossing Distributor") + theme_classic()

holidaygraph <- imdb_movies %>% 
  ggplot(aes(x = HolidaySeason, y = log(domestic))) +
  geom_boxplot() +
  labs(y="", x="Holiday Season") + theme_classic()
...
```{r data-viz-4, echo=FALSE, warning = FALSE, message = FALSE}
mpaagraph <- imdb_movies %>% 
  ggplot(aes(x = mpaa_factor, y = log(domestic))) +
  geom_boxplot() +
  labs(y="", x="MPAA Rating") + theme_classic()

actorgraph <- imdb_movies %>% 
  ggplot(aes(x = A_power, y = log(domestic))) +
  geom_boxplot() +
  labs(y="Logged Domestic Box Office Revenue", x="Number of Top 100 Grossing Actors") + 
  theme_classic()

ggarrange(actorgraph, mpaagraph,
          ncol = 2, nrow = 1)

imdb_movies$mpaa_factor <- factor(imdb_movies$mpaa_factor, levels=c("G", 'PG', 'PG-13', 'R', 'NC-17', "NA"))
...
```{r, loo-create, echo=TRUE, warning=FALSE, message=FALSE, eval=FALSE}
loo_model_1 <- loo(model_1)
loo_model_2 <- loo(model_2)
```


```{r, loo-eval, echo=TRUE, eval=FALSE}
...
```{r, message=FALSE, warning=FALSE, echo = FALSE, output = FALSE, results='hide'}
imdb_movies_new <- read.csv("/Users/alexmccreight/Portfolio/alex-hugosite/content/project/2021-12-17-Bayes/imdb_movies_new.csv")
imdb_movies_new <- 
  imdb_movies_new %>% 
